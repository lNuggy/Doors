local library = loadstring(game:HttpGet('https://raw.githubusercontent.com/lNuggy/Doors/main/library.lua'))()
        ...
        local function check(v)
            if v:IsA("Model") then
                ...
                if goldvalue and goldvalue >= flags.goldespvalue then
                    local hitbox = v:WaitForChild("Hitbox")
                    local h = esp(hitbox:GetChildren(), Color3.fromRGB(255, 255, 0), hitbox,
                        "GoldPile [" .. tostring(goldvalue) .. "]")
...
if game.ReplicatedStorage:WaitForChild("GameData"):WaitForChild("Floor").Value == "Rooms" then
    ...
    window_rooms.toggle("harmless A90", false, function(val)
        ...
        if val then
            local jumpscare = plr.PlayerGui:WaitForChild("MainUI"):WaitForChild("Jumpscare"):FindFirstChild(
                "Jumpscare_A90")
